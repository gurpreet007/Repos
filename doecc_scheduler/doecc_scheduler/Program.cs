...
namespace doecc_scheduler
{
    class Program
    {
        ...
        private void SendReconUptoEmail()
        {
            ...
            //call SendEMail
            SendEmail(FROM_ADDRESS, DISP_NAME, "gurpreet007@gmail.com", subject, content);
        }
        ...
        private void Process_Schedule(int sid, string dtFrom, string dtTo, string circode, string cirname, string cat)
        {
            ...
            sql = "SELECT SUBSTR(TRIM(ACNO),1,1) " +
                                        ...
                                        "|| DECODE(TRIM(TBL_NAME), 'DSBELOW10KW','DS','DSABOVE10KW','GC',TRIM(CATEGORY)) " +
        ...
        private void Do_All_Schedules()
        {
            ...
            foreach (DataRow schd in ds_schedules.Tables[0].Rows)
            {
                ...
                if (int.Parse(dtTo) <= int.Parse(dtLCMMO))
                {
                    Process_Schedule(sid, dtFrom, dtTo, circode, cirname, cat);
                }
                else
                {
                    contentPending = string.Format("SID {0}, Circle {1} ({2}), Category {3} skipped due to pending recon. To: {4}, Recon: {5}",
                    ...
                    SendEmail(FROM_ADDRESS, DISP_NAME, "gurpreet007@gmail.com", "Warning: Pending Recon", contentPending);
                }
            }
        }
